<template>
        <main class="main">
                ...
                <div class="card">
                    <div class="card-header">
                        ...
                        <button type="button" @click="abrirModal('transferencia','registrar')" class="btn btn-secondary">
                            <i class="icon-plus"></i>&nbsp;Directo
                        </button>
                        <button type="button" @click="abrirModal('transferencia','registrar2')" class="btn btn-secondary">
            ...
            <!--Inicio del modal agregar administrador-->
                        ...
                        <div class="modal-body">
                            <form action="" method="post" enctype="multipart/form-data" class="form-horizontal">
                                <div class="form-group row">
                                    <label class="col-md-3 form-control-label" for="email-input">Bodega salida</label>
                                    <div class="col-md-9">
                                    <select class="form-control col-md-3" v-model="idbodegasalida">
                                      <option value="0">Seleccione Bodega</option>
                                      <option v-for="bodega in arrayBodegasalida" :key="bodega.id" :value="bodega.id" v-text="bodega.alias"></option>
                                    </select>
                                    </div>
                                </div>
                                <div class="form-group row">
                                    ...
                                    <select class="form-control col-md-3" v-model="idbodegaentrada">
                                      <option value="0">Seleccione Bodega</option>
                                      <option v-for="bodega in arrayBodegaentrada" :key="bodega.id" :value="bodega.id" v-text="bodega.alias"></option>
                                    </select>
                                    </div>
                                </div>
                                <div class="form-group row">
            ...
            <!--Fin del modal-->
            <!--Inicio del modal agregarnormal-->
            <div class="modal fade" tabindex="-1" :class="{'mostrar' : modal2}" role="dialog" aria-labelledby="myModalLabel" style="display: none;" aria-hidden="true">
                <div class="modal-dialog modal-primary modal-lg" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" @click="cerrarModal()">Cerrar</button>
                            <button type="button" class="btn btn-primary" @click="registrarTransferencia2()">Guardar</button>
                        </div>
                    </div>
                    <!-- /.modal-content -->
                </div>
    ...
    export default {
        ...
        methods : {
            ...
            seleccionarBodega(){
                ...
                axios.get(url).then(function (response) {
                    var respuesta= response.data;
                    me.arrayBodegaentrada = respuesta.bodegas;
                    me.arrayBodegasalida = respuesta.bodegas;
                })
                .catch(function (error) {
                    console.log(error);
                });
            ...
            registrarTransferencia(){
               ...
               swal({
                ...
                if (result.value) {
                    ...
                    let me = this;
                    axios.post(this.ruta + '/transferir/registraradmin',{
                ...
                switch(modelo){
                    case "transferencia":
                    {
                        switch(accion){
                            ...
                            {
                                this.modal = 1;
                                this.tituloModal = 'Transferencia Directa';
                                this.idbodegaentrada = 0;
                                this.idbodegasalida = 0;
                                break;
                            }
                            case 'registrar2':
                            {
                                this.modal2 = 1;
                                this.tituloModal = 'Transferencia';
                                this.idbodegaentrada = 0;
                                this.idbodegasalida = 0;
                                break;
                            }
            ...
            validarTransferencia(){
                ...
                if (!this.idbodegaentrada) this.errorMostrarMsjTransferencia.push("La bodega entrada no debe estar vacio.");
            ...
            validarTransferencia2(){
                this.errorTransferencia=0;
                this.errorMostrarMsjTransferencia =[];
                if (!this.idbodegasalida) this.errorMostrarMsjTransferencia.push("La bodega salida no debe estar vacio.");         
                if (!this.idbodegaentrada) this.errorMostrarMsjTransferencia.push("La bodega entrada no debe estar vacio.");
                if (!this.cantidad) this.errorMostrarMsjTransferencia.push("La cantidad no debe estar vacio.");
                if (this.errorMostrarMsjTransferencia.length) this.errorTransferencia = 1;
